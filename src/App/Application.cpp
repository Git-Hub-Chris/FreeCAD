...
Document* Application::newDocument(const char * Name, const char * UserName, DocumentOpenFlags OpenFlags)
{
    ...
    name = getUniqueDocumentName(name.c_str(), OpenFlags.temporary);

    // return the temporary document if it exists
    if (OpenFlags.temporary) {
    ...
    newDoc->setStatus(Document::TempDoc, OpenFlags.temporary);
    ...
    signalNewDocument(*_pActiveDoc, OpenFlags.createView);
    ...
    if (OpenFlags.temporary && oldActiveDoc)
...
Document* Application::openDocument(const char * FileName, DocumentOpenFlags openFlags) {
    std::vector<std::string> filenames(1,FileName);
    auto docs = openDocuments(filenames, nullptr, nullptr, nullptr, openFlags);
    if(!docs.empty())
        return docs.front();
    return nullptr;
}
    ...
    do {
        ...
        for (std::size_t count=0;; ++count) {
            ...
            try {
                ...
                auto doc = openDocumentPrivate(path, name.c_str(), label, isMainDoc, openFlags, std::move(objNames));
...
Document* Application::openDocumentPrivate(const char * FileName,
        ...
        bool isMainDoc, DocumentOpenFlags openFlags,
        std::vector<std::string> &&objNames)
{
    ...
    Document* newDoc = newDocument(name.c_str(), label, openFlags);
