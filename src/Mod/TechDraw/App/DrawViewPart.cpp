    ...
    if (newFaceFinder()) {
        ...
        if (sortedWires.empty()) {
            Base::Console().Warning(
                "DVP::extractFaces - {} - Can't make faces from projected edges\n",
                getNameInDocument());
        }
        else {
...
TechDraw::BaseGeomPtr DrawViewPart::getGeomByIndex(int idx) const
{
    ...
    if ((unsigned)idx >= geoms.size()) {
        Base::Console().Error("DVP::getGeomByIndex({}) - invalid index - size: {}\n", idx,
                              geoms.size());
        return nullptr;
    }
    return geoms.at(idx);
...
TechDraw::VertexPtr DrawViewPart::getProjVertexByIndex(int idx) const
{
    ...
    if ((unsigned)idx >= geoms.size()) {
        Base::Console().Error("DVP::getProjVertexByIndex({}) - invalid index\n", idx);
        return nullptr;
    }
    return geoms.at(idx);
}
...
{
    //    Base::Console().Message("DVP::projectWire() - inWire.IsNull: {}\n", inWire.IsNull());
...
Base::Vector3d DrawViewPart::getLegacyX(const Base::Vector3d& pt, const Base::Vector3d& axis,
                                        const bool flip) const
{
    //    Base::Console().Message("DVP::getLegacyX() - {}\n", Label.getValue());
...
{
    //    Base::Console().Message("DVP::addReferencesToGeom() - {}\n", getNameInDocument());
    std::vector<TechDraw::VertexPtr> gVerts = getVertexGeometry();
...
{
    //    Base::Console().Message("DVP::addReferenceVertex({}) - {}\n",
...
{
    //    Base::Console().Message("DVP::add1CVToGV({})\n", tag.c_str());
    TechDraw::CosmeticVertex* cv = getCosmeticVertex(tag);
    if (!cv) {
        Base::Console().Message("DVP::add1CVToGV - cv {} not found\n", tag.c_str());
        return 0;
    }
    int iGV = geometryObject->addCosmeticVertex(cv->scaled(getScale()), cv->getTagAsString());
    cv->linkGeom = iGV;
...
{
    //    Base::Console().Message("DVP::getCVIndex({})\n", tag.c_str());
    std::vector<TechDraw::VertexPtr> gVerts = getVertexGeometry();
    ...
    i = 0;
    for (auto& cv : cVerts) {
        //        Base::Console().Message("DVP::getCVIndex - cv tag: {}\n",
    ...
    //    Base::Console().Message("DVP::getCVIndex - returns: {}\n", result);
    return -1;
}
...
{
    //    Base::Console().Message("CEx::add1CEToGE({}) 2\n", tag.c_str());
...
{
    //    Base::Console().Message("CEx::add1CLToGE({}) 2\n", tag.c_str());
    TechDraw::CenterLine* cl = getCenterLine(tag);
    if (!cl) {
        Base::Console().Message("CEx::add1CLToGE 2 - cl {} not found\n", tag.c_str());
        return -1;
    }
