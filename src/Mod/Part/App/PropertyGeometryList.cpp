...
void PropertyGeometryList::trySaveGeometry(Geometry * geom, Base::Writer &writer) const
{
    ...
    try {
        ...
        Base::Console().Warning(std::string("PropertyGeometryList"), "Not yet implemented: {}\n", e.what());
    }
}
...
void PropertyGeometryList::Restore(Base::XMLReader &reader)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if(reader.testStatus(Base::XMLReader::ReaderStatus::PartialRestoreInObject)) {
            Base::Console().Error("Geometry \"{}\" within a PropertyGeometryList was subject to a partial restore.\n",reader.localName());
            if(isOrderRelevant()) {
